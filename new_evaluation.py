...
_MODELS = {
    'binary_classification': [ # 114
         {
             'class': DecisionTreeClassifier, # 36
             'kwargs': {
                 'max_depth': [4, 8, 16, 32], 
         ...
         {
             'class': AdaBoostClassifier, # 3
             'kwargs': {
                 'n_estimators': [10, 50, 100]
             }
         },
         {
            'class': LogisticRegression, # 27
            'kwargs': {
                 'solver': ['lbfgs'],
                 'n_jobs': [-1],
                 'max_iter': [10, 50, 100],
        ...
        {
            'class': RandomForestClassifier, # 12
            'kwargs': {
                 'max_depth': [8, 16, None], 
    ...
    'regression': [ # 48
        ...
        {
            'class': RandomForestRegressor, # 12
            'kwargs': {
                 'max_depth': [8, 16, None], 
                 'min_samples_split': [2, 4],
                 'min_samples_leaf': [1, 3],
...
def _evaluate_multi_classification(train, test, metadata):
    ...
    for i in train:
        x_train, y_train, x_test, y_test, classifiers = _prepare_ml_problem(i, test, metadata)
...
def _evaluate_regression(train, test, metadata):
    ...
    x_trains, y_trains, x_tests, y_tests = [], [], [], []
    for i in train:
        x_train, y_train, x_test, y_test, regressors = _prepare_ml_problem(i, test, metadata)
        x_trains.append(x_train)
        y_trains.append(y_train)
        x_tests.append(x_test)
        y_tests.append(y_test)
        ...
        performance.append(
            {
                ...
                "mean_squared_error" : results.mean_squared.max(),
                "mean_absolute_error" : results.mean_absolute.max()
            }
        )
...
def compute_scores(test, synthesized_data, metadata):
    score = _EVALUATORS[metadata['problem_type']](synthesized_data, test, metadata) 
    return score
